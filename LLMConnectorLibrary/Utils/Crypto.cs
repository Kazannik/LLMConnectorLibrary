...
namespace LLMConnectorLibrary.Utils
{
	internal class Crypto
	{
		...
		private readonly RSACryptoServiceProvider? provider = PemKeyUtils.GetRSAProviderFromPemFile(@"public_key.pem");

		string RSA(string input)
		{

			return (Convert.ToBase64String(provider.Encrypt(Encoding.ASCII.GetBytes(input), false)));
		}
		...
		public class PemKeyUtils
		{
			...
			static byte[] GetOpenSSL3deskey(byte[] salt, SecureString secpswd, int count, int miter)
			{
				...
				for (int j = 0; j < miter; j++)
				{
					...
					{
						Array.Copy(result, hashtarget, result != null ? result.Length : 0);
						Array.Copy(data00, 0, hashtarget, result != null ? result.Length : 0, data00.Length);
						result = hashtarget;
				...
				Array.Clear(result, 0, result != null ? result.Length : 0);
